...
//        url = @"http://usvsolutions.com/lottery/totoResult/getLatestResult";
//    }
//    else if (getResultType == GetPrevious) {
//        url = @"http://usvsolutions.com/lottery/totoResult/getPrevious/date/";
//        url = [url stringByAppendingString:[self.resultSet getResultDate]];
//    }
//    else if (getResultType == GetNext) {
//        url = @"http://usvsolutions.com/lottery/totoResult/getNext/date/";
//        url = [url stringByAppendingString:[self.resultSet getResultDate]];
//    }
...
- (void)updateUI{
    ...
    for (int i=0; i<self.resultSet.winningNumbers.count - 1; i++) {
        win_num = [self.resultSet.winningNumbers objectAtIndex:i];
        if (win_num == (id)[NSNull null] || win_num.length == 0 ) win_num = @"";

        temp = [temp stringByAppendingString:win_num];
        temp = [temp stringByAppendingString:@"   "];
    }
    if ([self.resultSet.winningNumbers count] >=5)
    {
